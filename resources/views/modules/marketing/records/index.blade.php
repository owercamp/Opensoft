...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-4">
				<button type="button" title="Consultar seguimientos realizados" class="bj-btn-table-add form-control-sm historyOpportunity-link">HISTORIALES</button>
			</div>
			<div class="col-md-4">
						...
						<div class="col-md-6">
							<div class="form-group">
								<select name="marId_query" title="RAZON SOCIAL: (Oportunidades de negocio)" class="form-control form-control-sm" required>
									...
									@foreach($marketings as $marketing)
										<option value="{{ $marketing->marId }}"
											data-date="{{ $marketing->marDate }}"
											data-reason="{{ $marketing->marReason }}"
											data-municipality="{{ $marketing->munName }}"
											data-address="{{ $marketing->marAddress }}"
											data-contact="{{ $marketing->marContact }}"
											data-phone="{{ $marketing->marPhone }}"
											data-email="{{ $marketing->marEmail }}"
											data-observation="{{ $marketing->marObservation }}"
										>{{ $marketing->marReason . ' - ' . $marketing->munName }}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-md-6 text-center">
							<button type="button" class="bj-btn-table-add form-control-sm btn-history">CONSULTAR</button>
						</div>
					</div>
