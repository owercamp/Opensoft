...
<div class="col-md-12">
  <div class="row">
    ...
    <div class="col-md-6">
      <h6>ESTADISTICA DE ESTADOS - ORDENES DE SERIVICIO</h6>
  ...
  <div class="row sectionToPdf">
    <div class="col-md-12">
      <div class="row">
        <div class="col-md-6 content-schudeling">
          <canvas id="statisticOrders" width="300" height="150"></canvas>
        </div>
        <div class="col-md-6 content-schudeling">
          <table id="tblOrders" class="table table-hover text-center mt-4 border-top">
            <thead>
              <tr>
                <th></th>
                <!-- <th>TOTAL</th> -->
...
<script>
  ...
  var canvas = document.getElementById('statisticOrders');
  var ctx = document.getElementById('statisticOrders').getContext('2d');
  ...
  $('.btn-drawPdf').on('click', function(e) {
    ...
    var pdf = new jsPDF();
    pdf.setFontSize(15);
    pdf.text('ORDENES DE SERVICIO DEL AÑO ' + $('button.year').text(), 30, 25);
    pdf.addImage(img, 'png', 20, 40, 170, 90);
    pdf.save("ORDENES_" + $('button.year').text() + "_GENERADO EL " + Date() + ".pdf");
  });
  ...
  $('.btn-less').on('click', function(e) {
    ...
    if (year <= parseInt(yearnow)) {
      ...
      $.ajax({
        type: 'GET',
        url: "{{ route('occasional.statistic.vigente') }}",
        data: {
          year: year
        },
        dataType: 'json',
      ...
      $.ajax({
        type: 'GET',
        url: "{{ route('occasional.statistic.terminado') }}",
        data: {
          year: year
        },
        dataType: 'json',
