			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<a href="#" title="ANULAR" class="bj-btn-table-delete form-control-sm cancelOrder-link">
						...
						<a href="#" title="APROBAR" class="bj-btn-table-add form-control-sm aprovedOrder-link">
										...
										<div class="form-group">
											...
											<select name="oroDocument_id" class="form-control form-control-sm" required>
												...
												@foreach($documents as $document)
													<option value="{{ $document->docId }}"
														data-code="{{ $document->docCode }}"
														data-version="{{ $document->docVersion }}"
														>
														{{ $document->docName }}
													</option>
												@endforeach
										...
										<div class="form-group">
											...
											<select name="oroClientproposal_id" class="form-control form-control-sm" required>
												...
												@foreach($clients as $client)
													<option value="{{ $client->cprId }}"
														data-client="{{ $client->cprClient }}"
														data-document="{{ $client->cprNumberdocument }}"
														data-mail="{{ $client->cprEmail }}"
														data-contact="{{ $client->cprContact }}"
														data-phone="{{ $client->cprPhone }}"
														data-city="{{ $client->municipality->munName }}"
														data-briefcases="{{ $client->cprBriefcase }}"
													>
														{{ $client->cprClient }}
													</option>
												@endforeach
					...
					<form action="{{ route('occasional.order.update') }}" method="POST">
						...
						<div class="row border-top mt-3 text-center">
							<div class="col-md-6">
								...
								<button type="submit" class="bj-btn-table-edit form-control-sm my-3 btn-updateDefinitive">GUARDAR CAMBIOS</button>
							</div>
							<div class="col-md-6">
				...
				<div class="modal-body">
					...
					<div class="row mt-3 border-top text-center">
						<form action="{{ route('occasional.order.cancel') }}" method="POST" class="col-md-6">
							...
							<button type="submit" class="bj-btn-table-add form-control-sm my-3">ANULAR</button>
						</form>
						<div class="col-md-6">
							<button type="button" class="bj-btn-table-delete mx-3 form-control-sm my-3" data-dismiss="modal">CANCELAR</button>
						</div>
				...
				<div class="modal-body">
					...
					<div class="row mt-3 border-top text-center">
						<form action="{{ route('occasional.order.aproved') }}" method="POST" class="col-md-6">
							...
							<button type="submit" class="bj-btn-table-add form-control-sm my-3">APROBAR</button>
						</form>
						<div class="col-md-6">
							<button type="button" class="bj-btn-table-delete mx-3 form-control-sm my-3" data-dismiss="modal">CANCELAR</button>
						</div>
	...
	<script>
		...
		$('select[name=oroClientproposal_id]').on('change', function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getProposalFromOrderservice') }}",{ briefcases: briefcases },function(object){
		...
		$('select[name=oroDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getNextcodeForOrderoccasional') }}",{docId: selected},function(objectsNext){
					if(objectsNext != null){
				...
				$.get("{{ route('getContentFromDocumentComercial') }}",{docId: selected},function(objectsConfig){
		...
		$('.editOrder-link').on('click',function(e){
			...
			$('select[name=oroConfigdocument_id_Edit]').empty();
			$('select[name=oroConfigdocument_id_Edit]').append("<option value=''>Seleccione ...</option>");
			$.get("{{ route('getContentFromDocumentComercial') }}",{docId: oroDocument_id},function(objectsConfig){
				var count = Object.keys(objectsConfig).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
						if(objectsConfig[i]['cdoContent'].length > 100){
