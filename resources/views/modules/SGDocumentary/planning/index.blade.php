	...
	<script>
		...
		$('select[name=cddDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getVariablesFromDocumentDocumentary') }}",{dodId: selected},function(objectVariables){
		...
		$('.editDocument-link').on('click',function(e){
			Swal.fire({
				...
				if (result.isConfirmed) {					
					...
					$.get("{{ route('getVariablesFromDocumentDocumentary') }}",{dodId: cddDocument_id},function(objectVariables){
						var count = Object.keys(objectVariables).length;
						if(count > 0){
							for (var i = 0; i < count; i++) {
