...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-4">
				<button type="button" title="Registrar" class="bj-btn-table-add form-control-sm newLegalization-link">NUEVO</button>
			</div>
			<div class="col-md-4">
				...
				<tr>
					...
					<td>
						@if($legalization->lcoStatus != 'TERMINADO')
							<a href="#" title="Editar" class="bj-btn-table-edit form-control-sm editLegalization-link">
							...
							<a href="#" title="Eliminar" class="bj-btn-table-delete form-control-sm deleteLegalization-link">
						...
						<form action="{{ route('permanent.legalizations.pdf') }}" method="GET" style="display: inline-block;">
							...
							<button type="submit" title="Descargar PDF" class="bj-btn-table-add form-control-sm">
	...
	<script>
		...
		$('select[name=lcoDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getContentFromDocument') }}",{docId: selected},function(objectsConfig){
		...
		$('.editLegalization-link').on('click',function(e){
			...
			var contentAll = '';
			$.get("{{ route('getContentFromDocument') }}",{docId: lcoDocument_id},function(objectsConfig){
				var count = Object.keys(objectsConfig).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
						if(objectsConfig[i]['cdoContent'].length > 50){
