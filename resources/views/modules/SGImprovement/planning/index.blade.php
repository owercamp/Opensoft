	...
	<script>
		...
		$('select[name=cdiDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getVariablesFromDocumentImprovement') }}",{doIId: selected},function(objectVariables){
		...
		$('.editDocument-link').on('click',function(e){
			Swal.fire({
				...
				if (result.isConfirmed) {					
					...
					$.get("{{ route('getVariablesFromDocumentImprovement') }}",{doIId: cdiDocument_id},function(objectVariables){
						var count = Object.keys(objectVariables).length;
						if(count > 0){
							for (var i = 0; i < count; i++) {
