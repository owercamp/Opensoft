...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($exitexam->excDate)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $exitexam->excDocumentcode }}</td>
                </tr>
                <tr>
                    <td class="border">Versión</td>
                    <td class="border">{{ $exitexam->document->dolVersion }}</td>
                </tr>
                <tr>
                    <td class="border" rowspan="2">
                        {{ $exitexam->document->dolName }}                 
                    </td>
        ...
        <p style="text-align: justify; border-bottom-style: solid; border-color: #ccc;">
        	<b>{{ __('COLABORADOR: ') }}</b>{{ $exitexam->bill->collaborator->coNames }}<br>
        	<b>{{ __('CENTRO MEDICO: ') }}</b>{{ $exitexam->excCenter }}<br>
        </p>
        <h6>OBSERVACIONES:</h6>
        <p style="text-align: justify;">
            {{ $exitexam->excObservation }}
        </p>
