...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($examperiod->epcDate)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $examperiod->epcDocumentcode }}</td>
                </tr>
                <tr>
                    <td class="border">Versión</td>
                    <td class="border">{{ $examperiod->document->dolVersion }}</td>
                </tr>
                <tr>
                    <td class="border" rowspan="2">
                        {{ $examperiod->document->dolName }}                 
                    </td>
        ...
        <p style="text-align: justify; border-bottom-style: solid; border-color: #ccc;">
        	<b>{{ __('COLABORADOR: ') }}</b>{{ $examperiod->bill->collaborator->coNames }}<br>
        	<b>{{ __('CENTRO MEDICO: ') }}</b>{{ $examperiod->epcCenter }}<br>
        </p>
        <h6>OBSERVACIONES:</h6>
        <p style="text-align: justify;">
            {{ $examperiod->epcObservation }}
        </p>
