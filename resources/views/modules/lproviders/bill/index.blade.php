				...
				<div class="modal-body">
					...
					<div class="row mt-3 border-top text-center">
						<form action="{{ route('providers.bill.delete') }}" method="POST" class="col-md-6">
							...
							<button type="submit" class="bj-btn-table-pdf form-control-sm my-3">RECHAZAR</button>
						</form>
						<div class="col-md-6">
							<button type="button" class="bj-btn-table-delete mx-3 form-control-sm my-3" data-dismiss="modal">CANCELAR</button>
						</div>
	...
	<script>
		...
		$('select[name=bpDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getNextcodeForBillprovider') }}",{dolId: selected},function(objectsNext){
					if(objectsNext != null){
		...
		$('.editBill-link').on('click',function(e){
			...
			$.get("{{ route('getContentFromDocumentLogistic') }}",{dolId: bpDocument_id},function(objectsConfig){
				var count = Object.keys(objectsConfig).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
						if(objectsConfig[i]['cdlContent'].length > 100){
