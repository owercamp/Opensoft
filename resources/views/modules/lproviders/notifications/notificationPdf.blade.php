...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($notification->created_at)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $notification->npDocumentcode }}</td>
                </tr>
                <tr>
                    <td class="border">Versión</td>
                    <td class="border">{{ $notification->document->dolVersion }}</td>
                </tr>
                <tr>
                    <td class="border" rowspan="2">
                        {{ $notification->document->dolName }}                 
                    </td>
