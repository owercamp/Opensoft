...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($tracking->tcDate)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $tracking->tcDocumentcode }}</td>
                </tr>
                <tr>
                    <td class="border">Versión</td>
                    <td class="border">{{ $tracking->document->dolVersion }}</td>
                </tr>
                <tr>
                    <td class="border" rowspan="2">
                        {{ $tracking->document->dolName }}            
                    </td>
