...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            ...
            small.text-index:before { content: counter(contador-pag); }
            small.text-index { counter-increment: contador-pag; counter-reset: contador-total;}
        </style>
    </head>
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($training->trcDate)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $training->trcDocumentcode }}</td>
                </tr>
                <tr>
        ...
        <p style="text-align: justify; border-bottom-style: solid; border-color: #ccc;">
        	<b>{{ __('CAPACITACION: ') }}</b>{{ $training->trcNametraining }}<br>
        	<b>{{ __('CAPACITADOR: ') }}</b>{{ $training->trcNametrainer }}<br>
        </p>
        <table border="1" class=" table-striped text-center" width="100%" style="font-size: 11px; border-collapse: collapse;">
        	<thead>
	        	<tr>
	        		<th style="height: 20px; padding: 6px;">TIPO DE CONTRATISTA</th>
	        		<th style="height: 20px; padding: 6px;">CONTRATISTA</th>
        	...
        	<tbody>
        		@foreach($binnacles as $binnacle)
        			<tr>
						<td style="padding: 2px;">{{ $binnacle->bill->bcTypecontractor }}</td>
        				@if($binnacle->bill->bcTypecontractor == 'MENSAJERIA')
							<td style="padding: 2px;">{{ $binnacle->bill->messenger->cmNames }}</td>
							<td style="padding: 2px;">{{ $binnacle->bill->messenger->cmNumberdocument }}</td>
						@elseif($binnacle->bill->bcTypecontractor == 'CARGA EXPRESS')
							<td style="padding: 2px;">{{ $binnacle->bill->charge->ccNames }}</td>
							<td style="padding: 2px;">{{ $binnacle->bill->charge->ccNumberdocument }}</td>
						@elseif($binnacle->bill->bcTypecontractor == 'SERVICIO ESPECIAL')
							<td style="padding: 2px;">{{ $binnacle->bill->especial->ceNames }}</td>
							<td style="padding: 2px;">{{ $binnacle->bill->especial->ceNumberdocument }}</td>
						@endif
