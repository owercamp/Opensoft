...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-2">
				<button type="button" title="Registrar" class="bj-btn-table-add form-control-sm newTraining-link">NUEVO</button>
			</div>
			...
			<tbody>
				...
				<tr>
					...
					<td class="d-flex justofy-content-center">
						<a href="#" title="EDITAR" class="bj-btn-table-edit form-control-sm editTraining-link">
						...
						<a href="#" title="ELIMINAR" class="bj-btn-table-delete form-control-sm deleteTraining-link">
										...
										<div class="form-group">
											...
											<select name="trcLegalization_id" class="form-control form-control-sm">
												...
												@for($i = 0; $i < count($contractors); $i++)
													<option value="{{ $contractors[$i][0] }}" data-type="{{ $contractors[$i][1] }}" data-names="{{ $contractors[$i][2] }}" data-document="{{ $contractors[$i][3] }}">{{ $contractors[$i][1] . ' - ' . $contractors[$i][2] }}</option>
												@endfor
											</select>
										</div>
									</div>
									<div class="col-md-4 text-center">
										<button type="button" class="bj-btn-table-add form-control-sm mt-3 btn-addLegalization-newTraining" title='AGREGAR ASISTENTE'>Agregar asistente</button>
									</div>
								</div>
								<div class="row">
				...
				<div class="modal-body">
					<form action="{{ route('contractors.training.update') }}" method="POST">
						...
						<div class="row">
							<div class="col-md-12">
								...
								<div class="row">
									...
									<div class="col-md-4 text-center">
										<button type="button" class="bj-btn-table-add form-control-sm mt-3 btn-addLegalization-updateTraining" title='AGREGAR ASISTENTE'>Agregar asistente</button>
									</div>
								</div>
								<div class="row">
									<div class="col-md-12 p-3 text-center">
						...
						<div class="form-group text-center pt-2 border-top">
							...
							<button type="submit" class="bj-btn-table-edit form-control-sm btn-updateDefinitive">ACTUALIZAR</button>
						</div>
					</form>
				</div>
			</div>
	...
	<script>
		...
		$('select[name=trcDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getNextcodeForTrainingcontractor') }}",{dolId: selected},function(objectsNext){
					if(objectsNext != null){
				...
				if(validateRepet == false){
					...
					$('.tbl-legalizations-newTraining').find('tbody').append(
						"<tr class='" + bcId + "' data-idLegalization='" + bcId + "'>" +
							...
							"<td>" + number + "</td>" +
							"<td>" +
								"<button type='button' class='bj-btn-table-delete form-control-sm btn-deleteLegalization-newTraining' title='ELIMINAR ASISTENTE'><i class='fas fa-trash-alt'></i></button>" +
							"</td>" +
						"</tr>"
					);
				}else{
		...
		$('.editTraining-link').on('click',function(e){
			...
			$.get("{{ route('getLegalizationFromTrainingcontractor') }}",{trcId: trcId},function(objectsLegalizations){
					...
					for (var i = 0; i < count; i++) {
						$('.tbl-legalizations-updateTraining').find('tbody').append(
							"<tr class='" + objectsLegalizations[i][0] + "' data-idLegalization='" + objectsLegalizations[i][0] + "'>" +
								...
								"<td>" +
									"<button type='button' class='bj-btn-table-delete form-control-sm btn-deleteLegalization-updateTraining' title='ELIMINAR ASISTENTE'><i class='fas fa-trash-alt'></i></button>" +
								"</td>" +
							"</tr>"
						);
					}
