...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-4">
				<button type="button" title="Registrar" class="bj-btn-table-add form-control-sm newAgreement-link">NUEVO</button>
			</div>
			...
			<tbody>
				...
				<tr>
					...
					<td class="d-flex justofy-content-center">
						<a href="#" title="EDITAR" class="bj-btn-table-edit form-control-sm editAgreement-link">
						...
						<a href="#" title="ELIMINAR" class="bj-btn-table-delete form-control-sm deleteAgreement-link">
	...
	<script>
		...
		$('select[name=agcDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getNextcodeForAgreementcontractor') }}",{dolId: selected},function(objectsNext){
					if(objectsNext != null){
		...
		$('.editAgreement-link').on('click',function(e){
			...
			$.get("{{ route('getContentFromDocumentLogistic') }}",{dolId: agcDocument_id},function(objectsConfig){
				var count = Object.keys(objectsConfig).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
						if(objectsConfig[i]['cdlContent'].length > 100){
