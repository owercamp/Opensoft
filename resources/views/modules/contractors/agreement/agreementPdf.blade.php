...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body style="font-size: 12px;">
        @php $date = Date("Y-m-d",strtotime($agreement->created_at)) @endphp
        <header id="header">
            <table class="border" width="100%" style="text-align: center;">
                <tr>
                    ...
                    <td class="border">{{ $agreement->agcDocumentcode }}</td>
                </tr>
                <tr>
                    <td class="border">Versión</td>
                    <td class="border">{{ $agreement->document->dolVersion }}</td>
                </tr>
                <tr>
                    <td class="border" rowspan="2">
                        {{ $agreement->document->dolName }}            
                    </td>
