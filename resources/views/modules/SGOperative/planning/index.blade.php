	...
	<script>
		...
		$('select[name=cdoDocument_id]').on('change',function(e){
			...
			if(selected != ''){
				...
				$.get("{{ route('getVariablesFromDocumentOperative') }}",{doOId: selected},function(objectVariables){
		...
		$('.editDocument-link').on('click',function(e){
			Swal.fire({
				...
				if (result.isConfirmed) {					
					...
					$.get("{{ route('getVariablesFromDocumentOperative') }}",{doOId: cdoDocument_id},function(objectVariables){
						var count = Object.keys(objectVariables).length;
						if(count > 0){
							for (var i = 0; i < count; i++) {
