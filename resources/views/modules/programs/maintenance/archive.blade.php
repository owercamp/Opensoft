...
@section('space')
<div>
  <h5>ARCHIVO REVISION DE MANTENIMIENTO PREVENTIVO</h5>
</div>
  ...
  <table id="tableDatatable" class="table table-bordered table-striped text-center w-100">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td class="align-middle">{{substr($item->pmr_content,0,70).'...'}}</td>
        <td>
          <button class="btn btn-outline-primary rounded-circle showEdit"><i class="fas fa-edit"></i><span hidden aria-hidden="true">{{$item->pmr_id}}</span></button>
          <button class="btn btn-outline-danger rounded-circle mx-2 Delete-register"><i class="fas fa-eraser"></i><span hidden aria-hidden="true">{{$item->pmr_id}}</span><span hidden aria-hidden="true">{{$row-1}}</span></button>
          <button class="btn btn-outline-secondary rounded-circle PDF-programs"><i class="far fa-file-pdf"></i><span hidden aria-hidden="true">{{$item->pmr_id}}</span></button>
        </td>
      </tr>
      @endforeach
    </tbody>
  ...
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5> EDICION REVISION DE MANTENIMIENTO PREVENTIVO</h5>
        <button class="btn-close btn btn-dark btn-sm" data-dismiss="modal">&xvee;</button>
      </div>
      <div class="modal-body">
        <form action="{{route('update.maintenance')}}" method="post">
          @csrf @method('PATCH')
...
<form action="{{route('delete.maintenance')}}" method="post" id="formDelete" hidden aria-hidden="true">
  @csrf @method('DELETE')
  <input type="text" name="idDelete">
</form>

<form action="{{route('maintenance.pdf')}}" method="post" hidden aria-hidden="true" id="formPDF">
  @csrf
...
<script>
  ...
  $('.showEdit').click(function() {
    ...
    $.ajax({
      ...
      success(response) {
        $('Select[name=SelectDocument]').val(response[0]['pmr_config']);
        $('textarea[name=TextContent]').val(response[0]['pmr_content']);
        $('input[name=idhidden]').val(response[0]['pmr_id']);
      },
      complete() {
        $('#formEdit').modal();
      },
