...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-4">
				<button type="button" title="Registrar un proveedor" class="bj-btn-table-add form-control-sm newProvider-link">NUEVO</button>
			</div>
			<div class="col-md-4">
		...
		<table id="tableDatatable" class="table table-hover text-center" width="100%">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<a href="#" title="Editar proveedor {{ $provider->proReasonsocial }}" class="bj-btn-table-edit form-control-sm editProvider-link">
						...
						<a href="#" title="Eliminar proveedor {{ $provider->proReasonsocial }}" class="bj-btn-table-delete form-control-sm deleteProvider-link">
				...
				<div class="modal-body">
					<form action="{{ route('providers.providers.save') }}" method="POST">
						...
						<div class="form-group text-center">
							<button type="submit" class="bj-btn-table-add form-control-sm">REGISTRAR PROVEEDOR</button>
						</div>
					</form>
				</div>
			</div>
...
@section('scripts')
	<script>
		...
		$('.editProvider-link').on('click',function(e){
			...
			$.get("{{ route('getMunicipalities') }}",{ depId: proDepartment_id },function(objectMunicipalities){
				var count = Object.keys(objectMunicipalities).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
			...
			$.get("{{ route('getZonings') }}",{ munId: proMunicipality_id },function(objectZonings){
				var count = Object.keys(objectZonings).length;
				if(count > 0){
					for (var i = 0; i < count; i++) {
			...
			$.get("{{ route('getNeighborhoods') }}",{ zonId: proZoning_id },function(objectNeighborhoods){
