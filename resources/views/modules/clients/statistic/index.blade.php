			...
			<div class="col-md-12">
				<div class="row">
					<div class="col-md-8 content-schudeling">
						<canvas id="statisticClients" width="300" height="150"></canvas>
					</div>
					<div class="col-md-4">
...
@section('scripts')
	<script>
		...
		var canvas = document.getElementById('statisticClients');
		var ctx = document.getElementById('statisticClients').getContext('2d');
		...
		$('.btn-drawPdf').on('click',function(e){
			...
			var pdf = new jsPDF();
			pdf.setFontSize(15);
			pdf.text('REPORTE DE LICITACIONES Y PROPUESTAS DURANTE EL AÑO ' + $('button.year').text(), 30,25);
		...
		$('.btn-less').on('click',function(e){
			...
			if(year <= parseInt(yearnow)){
				...
				$.ajax({
					type: 'GET',
					url: "{{ route('clients.statistic.aproved') }}",
					data: {year: year},
					dataType: 'json',
					async: false
				}).done(function(response){
				...
				$.ajax({
					type: 'GET',
					url: "{{ route('clients.statistic.notaproved') }}",
					data: {year: year},
					dataType: 'json',
					async: false
				}).done(function(response){
