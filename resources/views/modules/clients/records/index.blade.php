...
@section('space')
	<div class="col-md-12 p-3">
		<div class="row border-bottom mb-3">
			...
			<div class="col-md-4">
				<button type="button" title="Consultar seguimientos realizados" class="bj-btn-table-add form-control-sm historyClient-link">HISTORIALES</button>
			</div>
			<div class="col-md-4">
							...
							<div class="form-group">
								<select name="cbiId_query" title="ENTIDAD (Licitaciones públicas)" class="form-control form-control-sm" required>
									...
									@foreach($biddingshistory as $bidding)
										<option value="{{ $bidding->cbiId }}"
											data-number="{{ $bidding->cbiNumberprocess }}"
											data-dateopen="{{ $bidding->cbiDateopen }}"
											data-dateclose="{{ $bidding->cbiDateclose }}"
											data-entity="{{ $bidding->cbiEntity }}"
											data-municipality="{{ $bidding->munName }}"
											data-modality="{{ $bidding->cbiModalitycontract }}"
											data-object="{{ $bidding->cbiObjectcontract }}"
											data-email="{{ $bidding->cbiEmail }}"
											data-observation="{{ $bidding->cbiObservation }}"
										>{{ $bidding->cbiEntity . ' - ' . $bidding->cbiNumberprocess }}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-md-6 text-center">
							<button type="button" class="bj-btn-table-add form-control-sm btn-history-bidding">CONSULTAR</button>
						</div>
					</div>
							...
							<div class="form-group">
								<select name="cprId_query" title="CLIENTE (Propuestas comerciales)" class="form-control form-control-sm" required>
									...
									@foreach($proposalshistory as $proposal)
										<option value="{{ $proposal->cprId }}"
											data-date="{{ $proposal->cprDate }}"
											data-client="{{ $proposal->cprClient }}"
											data-document="{{ $proposal->perName . '  N° ' . $proposal->cprNumberdocument }}"
											data-municipality="{{ $proposal->munName }}"
											data-modality="{{ $proposal->cprModalitycontract }}"
											data-email="{{ $proposal->cprEmail }}"
											data-phone="{{ $proposal->cprPhone }}"
											data-contact="{{ $proposal->cprContact }}"
											data-observation="{{ $proposal->cprObservation }}"
											data-briefcase="{{ $proposal->cprBriefcase }}"
										>{{ $proposal->cprClient . ' - N° ' . $proposal->cprNumberdocument }}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-md-6 text-center">
							<button type="button" class="bj-btn-table-add form-control-sm btn-history-proposal">CONSULTAR</button>
						</div>
					</div>
