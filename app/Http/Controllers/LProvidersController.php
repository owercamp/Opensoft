...
class LProvidersController extends Controller
{
  ...
  function saveBill(Request $request)
  {
    ...
    if ($validate == null) {
      ...
      for ($i = 0; $i < count($variablesSeparated); $i++) {
        ...
        switch ($var[1]) {
          case 'texto':
            $search = '/¡¡¡texto dinámico!!!/';
            break;
          case 'numero':
            $search = '/¡¡¡número dinámico!!!/';
            break;
          case 'moneda':
            $search = '/¡¡¡moneda dinámica!!!/';
            break;
          case 'calendario':
            $search = '/¡¡¡calendario dinámico!!!/';
            break;
        }
        if ($i == 0) {
  ...
  function testsTo()
  {
    $providers = DB::table('providers')->select()->get();
    return view('modules.lproviders.tests.index',compact('providers'));
  }

  function supplierTo(Request $request)
  {
    $data = Arr::except($request->all(),'_token');
    SupplierEvaluation::create($data);
    $name = Provider::find($data['su_provider']);
    return back()->with('Success','Se registro la evaluación del proveedor '.strtoupper($name['proReasonsocial']));
  }
