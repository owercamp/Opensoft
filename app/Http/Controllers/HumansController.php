...
class HumansController extends Controller
{
  ...
  function saveCollaborator(Request $request)
  {
    ...
    $academic = json_encode($request->academics);
    $title = json_encode($request->titles);
    $initial = json_encode($request->initials);
    $final = json_encode($request->finals);
    if ($validate == null) {
      if ($request->hasFile('coPhoto')) {
        if ($request->hasFile('coFirm')) {
          ...
          Collaborator::create([
            ...
            "addRef1" => $request->addRef1,
            "phoRef1" => trim($request->phoRef1),
            "ciuRef1" => ucfirst($request->ciuRef1),
            "rsRef2" => ucwords($request->rsRef2),
            "nitRef2" => trim($request->nitRef2),
            "addRef2" => $request->addRef2,
            "phoRef2" => trim($request->phoRef2),
            "ciuRef2" => ucfirst($request->ciuRef2),
            "titlePrimary" => ucfirst($request->titlePrimary),
            "acaPrimary" => ucfirst($request->acaPrimary),
            "dePrimary" => ucwords($request->dePrimary),
            "iniPrimary" => $request->iniPrimary,
            "finPrimary" => $request->finPrimary,
            "titleSecondary" => ucfirst($request->titleSecondary),
            "acaSecondary" => ucfirst($request->acaSecondary),
  ...
  {
    // dd($request->all());
    $validateOther = Collaborator::where([
      ['coNumberdocument', "=", trim($request->coNumberdocument)],
      ['coId', '!=', trim($request->coId_Edit)]
    ])->first();
    if ($validateOther == null) {
      ...
      if ($validate != null) {
        ...
        $validate->coNames = $this->fu($request->coNames);
        $validate->coPersonal_id = trim($request->coPersonal_id);
        $validate->coNumberdocument = $this->upper($request->coNumberdocument);
        $validate->coPosition = $this->fu($request->coPosition);
        $validate->coNeighborhood_id = trim($request->coNeighborhood_id);
        $validate->coAddress = $this->upper($request->coAddress);
        $validate->coEmail = $this->lower($request->coEmail);
        $validate->coMovil = $this->upper($request->coMovil);
        $validate->coWhatsapp = $this->upper($request->coWhatsapp);
        $validate->coBloodtype = $this->upper($request->coBloodtype);
        $validate->coHealths_id = trim($request->coHealths_id);
        $validate->coRisk_id = trim($request->coRisk_id);
        $validate->coCompensation_id = trim($request->coCompensation_id);
        $validate->coPension_id = trim($request->coPension_id);
        $validate->coLayoff_id = trim($request->coLayoff_id);
        $validate->colRef1 = ucwords($request->colRef1);
        $validate->cedRef1 = trim($request->cedRef1);
        $validate->numRef1 = trim($request->numRef1);
        $validate->colRef2 = ucwords($request->colRef2);
        $validate->cedRef2 = trim($request->cedRef2);
        $validate->numRef2 = trim($request->numRef2);
        $validate->rsRef1 = ucwords($request->rsRef1);
        $validate->nitRef1 = trim($request->nitRef1);
        $validate->addRef1 = strtoupper($request->addRef1);
        $validate->phoRef1 = trim($request->phoRef1);
        $validate->ciuRef1 = strtoupper($request->ciuRef1);
        $validate->rsRef2 = ucwords($request->rsRef2);
        $validate->nitRef2 = trim($request->nitRef2);
        $validate->addRef2 = strtoupper($request->addRef2);
        $validate->phoRef2 = trim($request->phoRef2);
        $validate->ciuRef2 = strtoupper($request->ciuRef2);
        $validate->titlePrimary = ucwords($request->titlePrimary);
        $validate->acaPrimary = ucwords($request->acaPrimary);
        $validate->dePrimary = strtoupper($request->dePrimary);
        $validate->iniPrimary = $request->iniPrimary;
        $validate->finPrimary = $request->finPrimary;
        $validate->titleSecondary = ucwords($request->titleSecondary);
        $validate->acaSecondary = ucwords($request->acaSecondary);
        $validate->deSecondary = strtoupper($request->deSecondary);
        $validate->iniSecondary = $request->iniSecondary;
        $validate->finSecondary = $request->finSecondary;
        $validate->academics = $academic;
        $validate->titles = $title;
        $validate->initials = $initial;
        $validate->finals = $final;
        $validate->save();
        return redirect()->route('humans.collaborators')->with('PrimaryCollaborator', 'Colaborador ' . $this->fu($request->coNames) . ', actualizado');
      } else {
  ...
  function contractorsExpressTo()
  {
    ...
    $drivings = Settingdriving::all();
    $courses = Settingcourse::all();
    return view('modules.humans.contractorsExpress.index', compact('contractorscharges', 'personals', 'deparments', 'healths', 'risks', 'compensations', 'pensions', 'layoffs', 'drivings', 'courses', 'date'));
  }
