...
class CreateReferencesExpressesTable extends Migration
{
  /**
   * Run the migrations.
   *
  ...
  public function up()
  {
    Schema::create('references_expresses', function (Blueprint $table) {
      $table->Increments('rc_id');
      $table->integer('rc_collaborator_id')->unsigned()->comment('se relaciona con el id del colaborador de la tabla Contractorschargeexpress');
      ...
      $table->foreign('rc_collaborator_id')->references('ccId')->on('contractorschargeexpress');
      $table->timestamps();
    });
  }
  ...
  {
    Schema::dropIfExists('references_expresses');
  }
}
